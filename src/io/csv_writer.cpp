...
[[nodiscard]] Result<void> StreamFailure(std::string_view detail = {}) {
...
[[nodiscard]] bool NeedsQuotes(std::string_view field, const CsvOptions& options) noexcept {
...
[[nodiscard]] bool CheckedAdd(std::size_t increment, std::size_t& total) noexcept {
  if (increment > std::numeric_limits<std::size_t>::max() - total) return false;
  total += increment;
  return true;
}

[[nodiscard]] Result<std::size_t> EncodedFieldSize(std::string_view field, const CsvOptions& options) {
  if (!NeedsQuotes(field, options)) return field.size();
...
[[nodiscard]] Result<std::size_t> EncodedRecordSize(std::span<const std::string_view> fields,
  ...
  for (std::size_t index = 0; index < fields.size(); ++index) {
    ...
    if (!field_size) return std::unexpected(field_size.error());
...
[[nodiscard]] Result<void> WriteCharacter(std::ostream& output, char character) {
  try {
    output.put(character);
  } catch (const std::ios_base::failure& failure) {
    return StreamFailure(failure.what());
  }
  if (!output) return StreamFailure();
  return {};
}

[[nodiscard]] Result<void> WriteText(std::ostream& output, std::string_view text) {
  std::size_t offset = 0;
...
Result<void> CsvWriter::WriteRow(std::span<const std::string_view> fields) {
  ...
  for (std::size_t index = 0; index < fields.size(); ++index) {
    ...
    if (!opening_quote) return opening_quote;
    for (const char character : field) {
    ...
    if (!closing_quote) return closing_quote;
  }
